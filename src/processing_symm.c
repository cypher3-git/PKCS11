...
/*
 * ==================================================================================
 * 模块名称: 对称加密处理 (Symmetric Cryptography Processing)
 * 文件功能: 实现对称密钥的加解密、签名验签操作
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. 对称加密/解密：AES各种模式、HMAC
 * 2. 对称签名/验签：HMAC、CMAC
 * 3. 密钥派生：支持的对称派生机制
 * 4. 多步处理：Init -> Update -> Final
 * 5. TEE集成：调用TEE Crypto API
 * 
 * 【支持的机制】
 * 加解密：
 * - AES-ECB:  电子密码本模式
 ...
 * - AES-CTS:  密文窃取模式
 * - AES-CCM:  Counter with CBC-MAC（带认证）
 * 
 * MAC（签名）：
 * - AES-CMAC:  密码消息认证码
 * - HMAC-MD5:  基于MD5的HMAC
 ...
 * 
 * 【处理流程】
 * 1. Init:   分配TEE操作句柄，设置密钥
 * 2. Update: 累积数据（可多次调用）
 * 3. Final:  完成操作并输出结果
 * 
 * 【核心函数】
 * - init_symm_operation():  初始化对称操作
 * - step_symm_operation():  执行对称操作步骤
 * - pkcs2tee_algorithm():   PKCS#11机制转TEE算法
 * - pkcs2tee_key_type():    PKCS#11密钥类型转TEE类型
 * 
 * 【AEAD模式】
 * AES-GCM/CCM支持：
 * - 关联数据(AAD)处理
 ...
 * 
 * 【安全特性】
 * - 密钥权限检查（CKA_ENCRYPT/DECRYPT/SIGN/VERIFY）
 * - 填充模式验证
 * - IV/Nonce唯一性（CTR/GCM模式）
 */
