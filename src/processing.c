...
/*
 * ==================================================================================
 * 模块名称: 密码处理引擎核心 (Processing Engine Core)
 * 文件功能: 密码操作的通用调度框架和生命周期管理
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. 处理调度：将PKCS#11命令分发到具体算法模块
 * 2. 状态管理：管理会话的处理状态机
 * 3. 生命周期：Init -> Update -> Final 三阶段处理
 * 4. 资源管理：TEE操作句柄的分配和释放
 * 5. 密钥管理：密钥生成、派生、包装/解包装
 * 6. 错误处理：统一的错误码转换和验证
 * 
 * 【处理类型】
 * - PKCS11_FUNCTION_ENCRYPT/DECRYPT: 加解密
 * - PKCS11_FUNCTION_SIGN/VERIFY:     签名验签
 * - PKCS11_FUNCTION_DIGEST:          消息摘要
 * - PKCS11_FUNCTION_GENERATE:        密钥生成
 * - PKCS11_FUNCTION_DERIVE:          密钥派生
 * - PKCS11_FUNCTION_WRAP/UNWRAP:     密钥包装
 * 
 * 【处理步骤】
 * - PKCS11_FUNC_STEP_INIT:      初始化操作
 * - PKCS11_FUNC_STEP_UPDATE:    更新数据（可多次调用）
 * - PKCS11_FUNC_STEP_FINAL:     完成操作并输出结果
 * - PKCS11_FUNC_STEP_ONESHOT:   单次完成（Init+Update+Final）
 * - PKCS11_FUNC_STEP_UPDATE_KEY: 更新密钥（用于C_DigestKey）
 * 
 * 【状态机】
 * READY → ENCRYPTING/DECRYPTING/SIGNING/VERIFYING/DIGESTING → READY
 *   ↑                                                              ↓
 *   └────────────────── Final/Cancel/Error ──────────────────────┘
 * 
 * 【算法模块分发】
 * - 对称加密 → processing_symm.c, processing_aes.c
 * - 非对称加密 → processing_asymm.c, processing_rsa.c
 * - 椭圆曲线 → processing_ec.c
 * - 摘要算法 → processing_digest.c
 * 
 * 【核心结构】
 * struct active_processing {
 *     enum pkcs11_proc_state state;    // 处理状态
 *     uint32_t mecha_type;              // 机制类型
 *     enum processing_step step;        // 当前步骤
 *     bool always_authen;               // 是否需要每次认证
 *     bool relogged;                    // 是否重新登录
 *     TEE_OperationHandle tee_op_handle; // TEE操作句柄
 *     TEE_OperationHandle tee_op_handle2; // 第二个操作句柄（用于组合操作）
 *     uint32_t tee_hash_algo;           // 哈希算法
 *     void *extra_ctx;                  // 扩展上下文
 * };
 * 
 * 【入口点】
 * - entry_processing_init(): 初始化处理（*_Init命令）
 * - entry_processing_step(): 执行处理步骤（*_Update/*_Final/*_Oneshot）
 * - entry_processing_key():  密钥相关处理（Derive/Unwrap）
 * - entry_generate_secret(): 生成对称密钥
 * - entry_generate_key_pair(): 生成密钥对
 * - entry_wrap_key():        包装密钥
 */
