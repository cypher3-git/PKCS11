...
/*
 * ==================================================================================
 * 模块名称: 非对称加密处理 (Asymmetric Cryptography Processing)
 * 文件功能: 实现非对称密钥的加解密、签名验签、密钥生成和包装操作
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. 非对称加密/解密：RSA (PKCS/OAEP/X.509)
 * 2. 非对称签名/验签：RSA-PSS, ECDSA, EdDSA
 * 3. 密钥对生成：RSA、EC、EdDSA密钥对
 * 4. 密钥派生：ECDH密钥协商
 * 5. 密钥包装：RSA-AES混合包装
 * 6. TEE集成：调用TEE Crypto API
 * 
 * 【支持的机制】
 * RSA加解密：
 * - CKM_RSA_PKCS:       PKCS#1 v1.5填充
 * - CKM_RSA_PKCS_OAEP:  OAEP填充（支持SHA-1/256/384/512）
 * - CKM_RSA_X_509:      无填充（原始RSA）
 * 
 * RSA签名：
 * - CKM_RSA_PKCS:       PKCS#1 v1.5签名
 * - CKM_RSA_PSS:        PSS填充签名
 * - CKM_MD5/SHA*_RSA_PKCS: 带哈希的RSA签名
 * 
 * 椭圆曲线：
 * - CKM_ECDSA:          ECDSA签名
 * - CKM_ECDSA_SHA*:     带哈希的ECDSA签名
 ...
 * 
 * 密钥生成：
 * - CKM_RSA_PKCS_KEY_PAIR_GEN:  RSA密钥对生成
 * - CKM_EC_KEY_PAIR_GEN:        EC密钥对生成
 * - CKM_EC_EDWARDS_KEY_PAIR_GEN: EdDSA密钥对生成
 * 
 * 密钥包装：
 * - CKM_RSA_AES_KEY_WRAP: RSA-AES混合密钥包装
 * 
 * 【处理流程】
 * 1. Init:   分配TEE操作句柄，设置密钥和参数
 * 2. Update: 累积数据（仅签名/验签支持）
 * 3. Final:  完成操作并输出结果
 * 
 * 【核心函数】
 * - init_asymm_operation():     初始化非对称操作
 * - step_asymm_operation():     执行非对称操作步骤
 * - generate_rsa_keys():        生成RSA密钥对
 * - generate_ec_keys():         生成EC密钥对
 * - derive_key_ecdh():          ECDH密钥派生
 * - wrap_rsa_aes_key():         RSA-AES混合包装
 * 
 * 【安全特性】
 * - 密钥权限检查
 * - 填充模式验证
