...
/*
 * ==================================================================================
 * 模块名称: 椭圆曲线算法处理 (Elliptic Curve Cryptography Processing)
 * 文件功能: 实现EC/EdDSA相关的参数处理、曲线定义和算法转换
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. 曲线管理：支持的EC曲线定义和DER编码参数
 * 2. 参数处理：ECDH参数解析和验证
 * 3. 算法映射：PKCS#11曲线类型转TEE曲线类型
 * 4. EdDSA支持：Ed25519参数处理
 * 
 * 【支持的曲线】
 * NIST曲线（P系列）：
 ...
 * 
 * 【ECDH参数】
 * - kdf:           密钥派生函数（仅支持CKD_NULL）
 * - sharedData:    共享数据
 * - publicData:    对端公钥
 * 
 * 【核心函数】
 * - ec_curve2tee_keytype():  PKCS#11曲线转TEE密钥类型
 * - tee_keytype2ec_curve():  TEE密钥类型转PKCS#11曲线
 * - pkcs2tee_proc_params_ecdh(): 处理ECDH参数
 * - pkcs2tee_proc_params_eddsa(): 处理EdDSA参数
 * - get_ec_tee_curve():      获取曲线的TEE_ECC_CURVE枚举
 * - get_ec_params():         获取曲线的DER编码参数
 * 
 * 【DER编码参数】
 * 每个曲线的DER编码参数预定义为静态数组，
 * 用于TEE密钥对象的CKA_EC_PARAMS属性。
 * 
 * 【安全特性】
 * - 曲线参数验证
 * - 公钥点验证（在曲线上）
 ...
 * DER编码的EC曲线参数
 * 
 * 使用脚本生成：ta/pkcs11/scripts/dump_ec_curve_params.sh
 * 
 * 这些参数用于设置EC密钥对象的CKA_EC_PARAMS属性，
 * 符合PKCS#11规范中定义的OID（对象标识符）格式。
 */
...
static const struct supported_ecc_curve *get_curve(void *attr, size_t size)
{
	...
	/* 简化版：不是真正的DER解析器：先尝试参数匹配，再尝试命名曲线 */
				...
				struct obj_attrs **priv_head)
{
	...
	/* 创建 ECDSA TEE 密钥：将匹配 PKCS11 的 ECDSA 和 ECDH */
