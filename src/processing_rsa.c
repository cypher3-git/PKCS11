...
/*
 * ==================================================================================
 * 模块名称: RSA算法处理 (RSA Cryptography Processing)
 * 文件功能: 实现RSA特定的参数处理和算法转换
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. RSA参数处理：PSS、OAEP参数解析和转换
 * 2. 算法映射：PKCS#11机制转TEE算法
 * 3. 参数验证：哈希算法、MGF、盐长度验证
 * 4. 上下文管理：RSA特定的处理上下文
 * 
 * 【支持的机制参数】
 * RSA-PSS参数：
 * - hashAlg:  哈希算法（SHA-1/224/256/384/512）
 * - mgf:      掩码生成函数（MGF1）
 * - sLen:     盐长度
 * 
 * RSA-OAEP参数：
 ...
 * 
 * 【处理上下文】
 * - rsa_pss_processing_ctx:  PSS签名上下文
 * - rsa_oaep_processing_ctx: OAEP加密上下文
 * - rsa_aes_key_wrap_processing_ctx: 密钥包装上下文
 * 
 * 【核心函数】
 * - pkcs2tee_proc_params_rsa_pss():  处理PSS参数
 * - pkcs2tee_proc_params_rsa_oaep(): 处理OAEP参数
 * - pkcs2tee_algo_rsa_pss():         PSS算法转换
 * - pkcs2tee_algo_rsa_oaep():        OAEP算法转换
 * 
 * 【参数验证】
 * - 哈希算法合法性检查
 * - MGF类型验证（仅支持MGF1）
