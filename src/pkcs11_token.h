 ...
 *
 * @version - 当前未使用...
 * @label - pkcs11 格式的令牌标签，由客户端设置
 * @flags - pkcs11 令牌标志
 * @so_pin_count - 安全管理员登录失败计数器
 * @so_pin_salt - 存储 SO PIN 哈希中的盐值，未设置时为 0
 * @so_pin_hash - 存储 SO PIN 的哈希值
 * @user_pin_count - 用户登录失败计数器
 * @user_pin_salt - 存储用户 PIN 哈希中的盐值，未设置时为 0
 * @user_pin_hash - 存储用户 PIN 的哈希值
 */
...
/*
 * 令牌的运行时状态，符合 pkcs11 规范
 *
 * @state - Pkcs11 登录状态：公开、用户、SO 或自定义
 * @session_count - 已打开 Pkcs11 会话的计数器
 * @rw_session_count - 已打开 Pkcs11 读写会话的计数
 * @object_list - 令牌拥有的对象列表
 * @db_main - 持久化主数据库的易失性副本
 * @db_objs - 持久化对象数据库的易失性副本
 */
struct ck_token {
...
/*
 * 会话只能从初始化状态进入处理状态（加密、解密、摘要等）。
 * 会话必须返回初始化状态（从处理完成请求）才能进入另一个处理状态。
 */
enum pkcs11_proc_state {
	PKCS11_SESSION_READY = 0,		/* 无活动处理 */
	PKCS11_SESSION_ENCRYPTING,
	PKCS11_SESSION_DECRYPTING,
	PKCS11_SESSION_DIGESTING,
	PKCS11_SESSION_DIGESTING_ENCRYPTING,	/* 情况：C_DigestEncryptUpdate */
	PKCS11_SESSION_DECRYPTING_DIGESTING,	/* 情况：C_DecryptDigestUpdate */
	PKCS11_SESSION_SIGNING,
	PKCS11_SESSION_SIGNING_ENCRYPTING,	/* 情况：C_SignEncryptUpdate */
	PKCS11_SESSION_VERIFYING,
	PKCS11_SESSION_DECRYPTING_VERIFYING,	/* 情况：C_DecryptVerifyUpdate */
 ...
 * 会话中活动处理的上下文
 *
 * @state - 正在进行的活动处理功能或就绪状态
 * @mecha_type - 活动处理的机制类型
 * @always_authen - 如果用户需要在每次使用前登录则为 true
 * @relogged - 自上次操作更新后客户端已登录时为 true
 * @op_step - 上次活动操作步骤 - update、final 或 one-shot
 * @tee_op_handle - 活动加密操作的句柄或 TEE_HANDLE_NULL
 * @tee_op_handle2 - 特定操作的第二个句柄或 TEE_HANDLE_NULL
 * @tee_hash_algo - 哈希算法标识符
 * @extra_ctx - 活动处理的上下文
 */
 ...
 * Pkcs11 对象搜索上下文
 *
 * @attributes - 搜索的匹配属性列表（无搜索时为 null）
 * @count - 找到的匹配句柄数量
 * @handles - 匹配对象的句柄数组
 * @next - 要返回给 C_FindObject 的下一个对象句柄的索引
 */
 ...
 * @token - 会话所属的令牌
 * @handle - 发布给客户端的会话标识符
 * @object_list - 会话对象列表的条目
 * @state - R/W SO、R/W user、RO user、R/W public、RO public
 * @processing - 如果有，指向已初始化的处理上下文的引用
 * @find_ctx - 指向活动搜索上下文的引用（无活动搜索时为 null）
 */
