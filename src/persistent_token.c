...
/*
 * ==================================================================================
 * 模块名称: 令牌持久化存储 (Persistent Token Storage)
 * 文件功能: 实现PKCS#11令牌数据的持久化存储，基于TEE安全存储
 * ==================================================================================
 * 
 * 【模块职责】
 * 1. 令牌配置持久化：标签、PIN、标志等
 * 2. 对象持久化：令牌对象的属性和密钥数据
 * 3. UUID管理：为每个持久化对象分配唯一UUID
 * 4. 数据库管理：维护对象UUID列表
 * 5. 完整性保护：使用TEE安全存储的加密和认证
 * 
 * 【存储架构】
 * 每个令牌有两个持久化对象：
 * 
 ...
 *    - 令牌标签
 *    - 令牌标志
 *    - User PIN（SHA256哈希或TEE身份）
 *    - SO PIN（SHA256哈希或TEE身份）
 *    - 所有令牌对象的UUID列表
 * 
 * 2. object_<UUID> (对象数据)
 *    每个令牌对象一个文件：
 ...
 * token.db结构：
 * +---------------------------+
 * | token_persistent_main     | 令牌主数据
 * |   - label[32]             | 令牌标签
 * |   - flags                 | 令牌标志
 * |   - so_pin_hash[32]       | SO PIN哈希
 * |   - so_pin_count          | SO PIN重试计数
 * |   - user_pin_hash[32]     | User PIN哈希
 * |   - user_pin_count        | User PIN重试计数
 * +---------------------------+
 * | token_persistent_objs     | 对象列表
 * |   - count                 | 对象数量
 * |   - uuids[]               | UUID数组
 * +---------------------------+
 * 
 ...
 * 
 * 【UUID生成】
 * 对象UUID = SHA256(令牌ID || 时间戳 || 计数器)
 * 确保每个对象的UUID全局唯一
 * 
 * 【核心函数】
 * - create_object_uuid():        为新对象生成UUID
 * - get_persistent_objects_list(): 获取对象UUID列表
 * - update_persistent_objects_list(): 更新对象列表
 * - unregister_persistent_object(): 删除对象
 * - load_persistent_object_attributes(): 加载对象属性
 * - update_persistent_object_attributes(): 保存对象属性
 * - get_persistent_token_info(): 读取令牌配置
 * - update_persistent_token_info(): 保存令牌配置
 * 
 * 【安全特性】
 * - PIN以SHA256哈希存储
 * - 可选绑定TEE设备身份
 ...
 * 令牌持久化对象
 *
 * 每个持久化对象都通过一个UUID进行标识。
 * 持久化对象数据库存储已注册的UUID列表。对于每个UUID，
 * 预期在TA安全存储中有一个ID为"UUID"的文件。
 */
					    ...
					    size_t pin_size)
{
	...
	if (!pin) {
		...
		/* 解析 PIN ACL 字符串：<login type>:<client id> */
